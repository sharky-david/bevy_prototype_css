...
/*/// Wrapper type where the `auto` keyword can be used
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum NumericMaybeAuto<NumericType: Numeric>  {
    Auto,
    Numeric(NumericType),
}

impl<NumericType: Numeric> NumericMaybeAuto<NumericType>
    where NumericType: Clone {
    #[inline]
    pub fn auto() -> Self {
        Self::Auto
    }
    ...
    pub fn auto_eval(
        &self,
        func: impl FnOnce() -> NumericType
    ) -> NumericType {
        match self {
            Self::Numeric(len) => len.clone(),
            Self::Auto => func(),
        }
    }

    #[inline]
    pub fn non_auto(&self) -> Option<NumericType> {
        match self {
            Self::Numeric(len) => Some(len.clone()),
            Self::Auto => None,
        }
    }
    ...
    pub fn parse_maybe_auto<'i, 't>(
        ...
        parser: impl FnOnce(
            &mut Parser<'i, 't>,
        ) -> Result<NumericType, BevyCssParsingError<'i>>
    ) -> Result<Self, BevyCssParsingError<'i>> {
        if input.try_parse(|i| i.expect_ident_matching("auto")).is_ok() {
            Ok(Self::Auto)
        } else {
            Ok(Self::Numeric( parser(input)? ))
        }
    }
}

impl<NumericType: Numeric> Numeric for NumericMaybeAuto<NumericType> {
    #[inline]
    fn zero() -> Self {
        Self::Numeric(NumericType::zero())
    }

    #[inline]
    fn one() -> Self {
        Self::Numeric(NumericType::one())
    }
    ...
    fn is_zero(&self) -> bool {
        match *self {
            Self::Auto => false,
            Self::Numeric(ref num) => num.is_zero(),
        }
    }
    ...
    fn is_negative(&self) -> bool {
        match *self {
            Self::Auto => false,
            Self::Numeric(ref num) => num.is_negative(),
        }
    }
    ...
    fn is_infinite(&self) -> bool {
        match *self {
            Self::Auto => false,
            Self::Numeric(ref num) => num.is_infinite(),
        }
    }
}

impl<NumericType> Parse for NumericMaybeAuto<NumericType>
    where NumericType: Parse {
    #[inline]
    fn parse<'i, 't>(input: &mut Parser<'i, 't>) -> Result<Self, BevyCssParsingError<'i>> {
        Self::parse_maybe_auto(input, NumericType::parse)
    }
}*/